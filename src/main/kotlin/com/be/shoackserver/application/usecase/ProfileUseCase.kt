...
class ProfileUseCase(
    ...
    fun loadProfile() : ProfileResponse{
        val memberDto = MemberDto.of(memberService.getMember())
    ...
    fun updateProfileImage(multipartFile: MultipartFile) {
        // 기존 프로필 이미지 삭제
        memberService.getMember().imageName?.let { imageService.deleteImage("profile", it) } ?: throw IllegalStateException("Image name is null")
        val imageName = imageService.uploadMultipartFile(multipartFile, "profile")
        memberService.updateMemberProfileImageName(imageName)
